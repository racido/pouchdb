...
function fetchCheckpoint(src, target, id) {
...
function replicate(repId, src, target, opts, returnValue) {
  ...
  function getRevisionOneDocs() {
    if (opts.fetchGenerationOneRevs) {
      ...
      console.log('gen 1 ' + JSON.stringify(generationOne.ids) +
        ', ' + JSON.stringify(ids));
      return new utils.Promise(function (fulfill, reject) {
        fulfill();
      });
  ...
  function startNextBatch() {
    ...
    console.log('new currentBatch ' + JSON.stringify(currentBatch));
    getDiffs()
    .then(getDocs)
    .then(writeDocs)
    .then(finishBatch)
  ...
  function replicationComplete() {
    console.log('replicationComplete ' + JSON.stringify(result));
    if (completeCalled) {
      console.log('again... ignore');
      return;
    }
    if (returnValue.cancelled) {
  ...
  function startChanges() {
    fetchCheckpoint(src, target, repId).then(
    function (checkpoint) {
      last_seq = checkpoint;
      changesOpts = {
        since: last_seq,
    ...
    function (err) {
      abortReplication('fetchCheckpoint rejected with ', err);
    });
  }
